...
{
  ADDPKGS=""
  OPTS=$(dialog --backtitle "$BACKTITLE" --checklist "Choose additional package(s) to install:" 0 0 0 \
        1 "Issabel Network Licensed modules (http://issabel.guru)" off \
        2 "Issabel.guru community RBL (Realtime Blackhole List)" on \
  ...
  do
    case $i in
      1)
      ADDPKGS="$ADDPKGS issabel-license webconsole issabel-wizard issabel-packet_capture issabel-upnpc issabel-two_factor_auth issabel-theme_designer"
      ;;
      2)
...
function post_install
{
  ...
  mysql -e "SET PASSWORD FOR 'root'@'localhost' = PASSWORD('4n7ntB3ir893')"
  ...
  # # JMV # #
  #mysql -p4n7ntB3ir893 -e "SET PASSWORD FOR 'root'@'localhost' = PASSWORD('')"
...
{
  cat > /etc/yum.repos.d/Issabel-beta.repo <<EOF
[issabel-beta]
name=Beta RPM Repository for Issabel
#mirrorlist=http://mirror.issabel.org/?release=4&arch=$basearch&repo=beta
baseurl=http://repo.issabel.org/issabel/4/beta/$basearch/
